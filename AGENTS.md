...
- `useValue`, `useError`, `useTouched`, `useIsDirty`, `useDirtyFields`, `useTouchedFields`, `useHasErrors`, `useIsSubmitting`, `useSubmitCount`, `useIsValidating` (`form.validating.size > 0`: field async/debounce windows plus the form-level round), `useIsSubmitSuccessful`, `useFormError`/`useFormErrors` (read the `FORM_ERROR` key) — state hooks built on `useWatch`
...
`react-f0rm/server` — tree-shakeable entry importing only from `./form` (zero React in the module graph, RSC/Server Action safe): `validateValues(values, options?)` runs one whole-form validation round over a plain payload (`createForm` + `trigger`, never rejects) and returns `{valid, values, errors}` where `values` carries schema-coerced output (the `parsedValues` baseline) and `errors` is the flat `FieldErrorEntry[]` list; re-exports `VALIDATION_OUTCOME`/`ValidationOutcome` so branded outcomes can be built server-side. The TanStack `createServerValidate` counterpart minus the action prop — the values store stays the source of truth.
