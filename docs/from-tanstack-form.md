...
| `useTransform(form, name, {onChange})` | `useTransform(form, name, {toDisplay, fromDisplay})` | Two explicit directions: the store always holds the raw typed value, `toDisplay`/`fromDisplay` map between it and the control's display value (`useField`'s `isDirty`/`validating` are the reactive twins of TanStack's field `meta`). |
