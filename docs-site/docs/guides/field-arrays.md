...
Two bulk operations complement the movers (`append`, `prepend`, `insert`, `remove`, `swap`, `move`):
...
`replace(values)` is the refetch shape — a server response replaces the whole list, length may change — while `update(index, value)` overwrites a single value in place without churning React keys.
