...
| `validateOnMount` | `boolean` | Validate this field once on mount — overrides the form-level `createForm({validateOnMount})` / `<Form validateOnMount>` flag in either direction |
| `valueAsNumber` | `boolean` | Store `e.target.valueAsNumber` instead of the string value (RHF `register` option for `<input type="number">`); `NaN` passes through when the input cannot be parsed. `eventToValue` wins |
| `valueAsDate` | `boolean` | Store `e.target.valueAsDate` instead of the string value (RHF `register` option for date/time inputs); `null` passes through when the input cannot be parsed. `eventToValue` wins; combined with `valueAsNumber`, DEV warns and number wins |
